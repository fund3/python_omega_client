...
def test_handle_omega_message_system():
    ...
    system_msg = system_message_py(omega_mess.type.response.body.system)
...
def test_handle_omega_message_logon():
    # logon success
    omega_mess = msgs_capnp.TradeMessage.new_message()
    ...
    logon_ack = logon_ack_py(omega_mess.type.response.body.logonAck)
    ...
    # logon failure
    omega_mess1 = msgs_capnp.TradeMessage.new_message()
    logon_ack_resp1 = omega_mess1.init('type').init('response')
    ...
    expected_auth_grant = AuthorizationGrant(
        ...
        refresh_token="",
        expire_at=0.0)
    logon_ack = logon_ack_py(omega_mess1.type.response.body.logonAck)
    assert type(logon_ack.message.body) == str
...
def test_handle_omega_message_logoff():
    omega_mess = msgs_capnp.TradeMessage.new_message()
    logoff_resp = omega_mess.init('type').init('response')
    ...
    logoff.message.body = ('The Times 03/Jan/2009 Chancellor on brink of ' +
                           'second bailout for banks')
    logoff_ack = logoff_ack_py(omega_mess.type.response.body.logoffAck)
    ...
    omega_mess1 = msgs_capnp.TradeMessage.new_message()
    logoff_resp = omega_mess1.init('type').init('response')
    logoff_resp.clientID = 123
    logoff_resp.senderCompID = str(987)
    ...
    logoff_ack = logoff_ack_py(omega_mess1.type.response.body.logoffAck)
    assert type(logoff_ack.message.body) == str
...
def test_handle_omega_message_account_data_report():
    omega_mess = msgs_capnp.TradeMessage.new_message()
    account_data_resp = omega_mess.init('type').init('response')
    ...
    acct_data_report = account_data_report_py(
        account_data_report=omega_mess.type.response.body.accountDataReport)
...
def test_handle_omega_message_working_orders_report():
    omega_mess = msgs_capnp.TradeMessage.new_message()
    working_orders_resp = omega_mess.init('type').init('response')
    ...
    wos_reports = working_orders_report_py(
        working_orders_report=omega_mess.type.response.body.workingOrdersReport)
...
def test_handle_omega_message_account_balances_report():
    omega_mess = msgs_capnp.TradeMessage.new_message()
    account_balances_resp = omega_mess.init('type').init('response')
    ...
    acct_bals = account_balances_report_py(
        omega_mess.type.response.body.accountBalancesReport)
...
def test_handle_omega_message_completed_orders_report():
    omega_mess = msgs_capnp.TradeMessage.new_message()
...
def test_handle_omega_message_open_positions_report():
    omega_mess = msgs_capnp.TradeMessage.new_message()
    open_positions_resp = omega_mess.init('type').init('response')
    ...
    open_pos_report = open_positions_report_py(
        omega_mess.type.response.body.openPositionsReport)
...
def test_handle_omega_message_exchange_properties_report():
    # valid test case
    omega_mess = msgs_capnp.TradeMessage.new_message()
    exchange_properties_resp = omega_mess.init('type').init('response')
...
def test_handle_omega_message_execution_report():
    # order accepted
    omega_mess = msgs_capnp.TradeMessage.new_message()
    exec_report_resp = omega_mess.init('type').init('response')
    ...
    er_type = execution_report_py(omega_mess.type.response.body.executionReport)
    assert type(er_type) == ExecutionReport
    assert er_type.execution_report_type == 'orderAccepted'

    # order rejected
    omega_mess1 = msgs_capnp.TradeMessage.new_message()
    exec_report_resp1 = omega_mess1.init('type').init('response')
    ...
    er_type1 = execution_report_py(omega_mess1.type.response.body.executionReport)
    assert type(er_type1) == ExecutionReport
    assert er_type1.execution_report_type == 'orderRejected'

    # order replaced
    omega_mess2 = msgs_capnp.TradeMessage.new_message()
    exec_report_resp2 = omega_mess2.init('type').init('response')
    ...
    er_type2 = execution_report_py(omega_mess2.type.response.body.executionReport)
    assert type(er_type2) == ExecutionReport
    assert er_type2.execution_report_type == 'orderReplaced'

    # replace rejected
    omega_mess3 = msgs_capnp.TradeMessage.new_message()
    exec_report_resp3 = omega_mess3.init('type').init('response')
    ...
    er_type3 = execution_report_py(omega_mess3.type.response.body.executionReport)
    assert type(er_type3) == ExecutionReport
    assert er_type3.execution_report_type == 'replaceRejected'

    # order cancelled
    omega_mess4 = msgs_capnp.TradeMessage.new_message()
    exec_report_resp4 = omega_mess4.init('type').init('response')
    ...
    er_type4 = execution_report_py(omega_mess4.type.response.body.executionReport)
    assert type(er_type4) == ExecutionReport
    assert er_type4.execution_report_type == 'orderCanceled'

    # cancel rejected
    omega_mess5 = msgs_capnp.TradeMessage.new_message()
    exec_report_resp5 = omega_mess5.init('type').init('response')
    ...
    er_type5 = execution_report_py(omega_mess5.type.response.body.executionReport)
    assert type(er_type5) == ExecutionReport
    assert er_type5.execution_report_type == 'cancelRejected'

    # order filled
    omega_mess6 = msgs_capnp.TradeMessage.new_message()
    exec_report_resp6 = omega_mess6.init('type').init('response')
    ...
    er_type6 = execution_report_py(omega_mess6.type.response.body.executionReport)
    assert type(er_type6) == ExecutionReport
    assert er_type6.execution_report_type == 'orderFilled'

    # status update
    omega_mess7 = msgs_capnp.TradeMessage.new_message()
    exec_report_resp7 = omega_mess7.init('type').init('response')
    ...
    er_type7 = execution_report_py(omega_mess7.type.response.body.executionReport)
...
def test_handle_omega_message_test():
    ...
    test_string = omega_test_message_py(
        omega_mess.type.response.body.test)
...
def test_heartbeat_capnp():
    ...
    heartbeat_req = expected_omega_message.init('type').init('request')
    ...
    actual_omega_message = heartbeat_capnp(__FAKE_REQUEST_HEADER)[0]
    ...
    assert actual_omega_message.type.request.body.heartbeat == (
        expected_omega_message.type.request.body.heartbeat)


@pytest.mark.test_id(18)
def test_request_server_time_capnp():
    expected_omega_message = msgs_capnp.TradeMessage.new_message()
    request_server_time = expected_omega_message.init('type').init('request')
    ...
    actual_omega_message = request_server_time_capnp(__FAKE_REQUEST_HEADER)[0]
    ...
    assert actual_omega_message.type.request.requestID == (
        expected_omega_message.type.request.requestID)
    assert actual_omega_message.type.request.body.getServerTime == (
        expected_omega_message.type.request.body.getServerTime)
...
def test_request_auth_refresh_capnp():
    ...
    expected_omega_message = msgs_capnp.TradeMessage.new_message()
    request_auth_refresh = expected_omega_message.init('type').init('request')
    ...
    actual_omega_message = request_auth_refresh_capnp(__FAKE_REQUEST_HEADER,
                                                    py_auth_refresh)[0]
    ...
    actual_auth_refresh_token = (
        actual_omega_message.type.request.body.authorizationRefresh.refreshToken)
    expected_auth_refresh_token = (
        expected_omega_message.type.request.body.authorizationRefresh.refreshToken
    )
